...
8. **`.agents/workflow.yml` (Multi-Agent Structure)**:
   ...
   - **Model Routing**: Do not rely on hardcoded model names. Perform a live web search to identify the *current* best state-of-the-art (SOTA) frontier models for complex coding (*Architecte*, *Développeur*) and the best lightweight/fast models for simple tasks (*Testeur QA*, *Product Owner*). Explicitly assign these recently discovered models in the workflow to optimize cost and performance.
